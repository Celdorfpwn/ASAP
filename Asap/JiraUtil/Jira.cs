...
namespace JiraService
{
    public class Jira : IIssuesTracking
    {
        ...
        public SearchResult GetIssues(string username = "", string project = "")
        {
            ...
            string query = String.Format("search?jql=status%20in%20(Open%2C%20'In%20Progress'%2C%20Reopened)%20AND%20assignee%20in%20('{0}')%20AND%20project%20%3D{1}+order+by+priority", username, project);
            string response = RunQuery(query);
